  ...
  return (
    <div className="min-h-screen bg-theme-background">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-theme-primary mb-6 sm:mb-8">Job Board</h1>
        <div className="flex overflow-x-auto gap-1 mb-6 sm:mb-8 bg-theme-surface rounded-lg p-1 shadow-sm scrollbar-hide">
          {['browse', 'post', 'applications'].map((tab) => (
            <button 
              key={tab}
              onClick={() => setActiveTab(tab)}
              className={`px-4 sm:px-6 py-2 rounded-md font-semibold transition-colors whitespace-nowrap text-sm sm:text-base ${
                activeTab === tab ? 'bg-theme-primary text-white' : 'text-theme-secondary hover:text-theme-primary'
              }`}
            >
              {tab === 'browse' ? 'Browse' : tab === 'post' ? 'Post' : 'Applications'}
            </button>
          ))}
        </div>
          ...
          <div className="lg:col-span-2 space-y-4 sm:space-y-6">
            <div className="flex flex-col gap-3 mb-4 sm:mb-6">
              <input 
                ...
                className="w-full px-4 py-3 border border-theme rounded-lg focus:ring-2 focus:ring-theme-primary focus:border-theme-primary bg-theme-surface text-theme-primary text-base" 
                value={searchTerm}
                onChange={(e) => setSearchTerm(e.target.value)}
              />
            </div>
            <p className="text-theme-secondary mb-4 text-sm sm:text-base">Showing {filteredJobs.length} jobs</p>
            {filteredJobs.map((job) => (
              <div key={job.id} className="card p-4 sm:p-6 hover:shadow-lg transition-all">
                <div className="flex flex-col gap-3 mb-4">
                  <div>
                    <h3 className="text-lg sm:text-xl font-bold text-theme-primary mb-2">{job.title}</h3>
                ...
                <div className="flex flex-col sm:flex-row gap-2">
                  <button 
                    ...
                    className={`btn px-4 sm:px-6 py-2 rounded-lg font-semibold transition-colors text-sm sm:text-base ${
                  ...
                  <button 
                    ...
                    className={`btn border px-4 sm:px-6 py-2 rounded-lg font-semibold transition-colors text-sm sm:text-base ${
