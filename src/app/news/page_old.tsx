  ...
  return (
    <div className="min-h-screen bg-theme-background">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-theme-primary mb-6 sm:mb-8">News & Updates</h1>
        <div className="flex overflow-x-auto gap-1 mb-6 sm:mb-8 bg-theme-surface rounded-lg p-1 shadow-sm scrollbar-hide">
          {['all', 'college', 'alumni'].map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)} className={`px-4 sm:px-6 py-2 rounded-md font-semibold transition-colors whitespace-nowrap text-sm sm:text-base ${
                activeTab === tab ? 'bg-theme-primary text-white' : 'text-theme-secondary hover:text-theme-primary'
              }`} suppressHydrationWarning={true}>
              {tab === 'all' ? 'All' : tab === 'college' ? 'College' : 'Alumni'}
            </button>
          ))}
        </div>
        <div className="grid lg:grid-cols-3 gap-6 sm:gap-8">
          <div className="lg:col-span-2 space-y-6 sm:space-y-8">
            {filteredNews.map((article) => (
              <article key={article.id} className="card p-4 sm:p-6 md:p-8 hover:shadow-lg transition-all">
                ...
                <h2 className="text-xl sm:text-2xl font-bold text-theme-primary mb-3 sm:mb-4">{article.title}</h2>
