  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      if (formData.studentId && formData.password) {
        ...
        setError('Please enter valid student credentials');
      }
      setIsLoading(false);
    }, 1000);
  };
        ...
        <div className="text-center mb-8">
          <Link href="/" className="inline-flex items-center space-x-3 mb-6">
            ...
            <div className="text-left">
              ...
              <span className="text-sm text-theme-secondary">Student Portal</span>
            </div>
          </Link>
          <h1 className="text-3xl font-bold text-theme-primary mb-3">Student Login</h1>
          <p className="text-theme-secondary text-lg">Access your student account</p>
        </div>

        <div className="card p-8 shadow-lg">
          <form onSubmit={handleSubmit} className="space-y-6">
            <div>
              <label className="block text-sm font-semibold text-theme-primary mb-2">Student ID</label>
              <input
                type="text"
                required
